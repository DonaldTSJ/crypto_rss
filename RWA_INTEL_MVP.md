...
这个项目现在采用 Supabase-first 数据流：Supabase 是团队共享数据中心，飞书负责提醒，Obsidian 可选同步本地 Markdown 镜像。
...
RSS/API/网页/公告
  ...
  -> 可选写入本地 Obsidian vault 的 RWA Intel 子目录
  -> 本地内部看板读取 Supabase
```
...
- 本地不再维护 SQLite 状态库；Obsidian 只作为可选 Markdown 同步目标，不作为主状态库。
- 信息源分为 `regulatory` 和 `message`：监管/SRO/CSD/CCP/官方规则源默认每周跑一次，非监管消息源默认每日跑。
...
rwa_intel_mvp/
  ...
  obsidian.py            # 可选 Obsidian Markdown 同步
...
```bash
python3 -m rwa_intel_mvp.cli run --source-class message --dry-run
python3 -m rwa_intel_mvp.cli run --source-class regulatory --all-dates --dry-run
```

同步本地 Obsidian：

```bash
python3 -m rwa_intel_mvp.cli run --use-deepseek --obsidian-sync --min-score 70
```

`--obsidian-sync` 只会在 `OBSIDIAN_VAULT_PATH/OBSIDIAN_FOLDER` 下创建新的时间戳 Markdown 文件，不删除或覆盖既有笔记。
...
```bash
python3 -m rwa_intel_mvp.cli schedule --frequency daily
python3 -m rwa_intel_mvp.cli schedule --frequency weekly --weekday mon --obsidian-sync
```
...
`category: "regulator"` 会默认推断为 `source_class: "regulatory"`；其他旧源默认推断为 `message`。官方 SRO、CSD/CCP、DTCC、HKEX HKSCC、DFSA/QFC rulebook 等源在 `default_sources.json` 中显式标为监管周频源。
